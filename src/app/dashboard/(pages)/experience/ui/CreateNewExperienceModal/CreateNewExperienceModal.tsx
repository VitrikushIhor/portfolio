...
import {useCreateExperienceMutation} from '@/app/dashboard/(pages)/experience/api/createExperience';
...
export default function CreateNewExperienceModal(props: InterfaceSkillModal) {
	...
	const onSubmit: SubmitHandler<InterfaceExperience> = async (data: any) => {
		...
		const res = await create({data: formData})
		if (res) {
			setIsOpen(false)
		}
	};
